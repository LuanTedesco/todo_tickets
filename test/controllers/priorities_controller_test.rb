...
class PrioritiesControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create priority" do
    assert_difference("Priority.count") do
      post priorities_url, params: { priority: { description: @priority.description, level: @priority.level, status: @priority.status, title: @priority.title } }
    end
  ...
  test "should update priority" do
    patch priority_url(@priority), params: { priority: { description: @priority.description, level: @priority.level, status: @priority.status, title: @priority.title } }
